...
sys.path.append("../")
from model_music_2 import TrellisNetModel
...
parser.add_argument('--aux', type=float, default=0.7,
                    help='use auxiliary loss (default: 0.5), -1 means no auxiliary loss used')
...
train_data_raw, train_label = get_data(args.batch_size,sessions_train,args.seq_len)
val_data_raw, val_label = get_data(args.batch_size,sessions_val,args.seq_len)
test_data_raw, test_label = get_data(args.batch_size,sessions_test,args.seq_len)
...
def train(epoch,data_source, label):
    ...
    for batch, i in enumerate(range(0, data_source.size(0) - 1, args.seq_len)):
        ...
        final_decoded = decoded[eff_history:].contiguous().view(-1, args.nout)
        ...
        if final_decoded.shape[0]:
            # Loss 1: CE loss
            raw_loss = criterion(final_decoded, track_weight[targets])
            ...
            if args.aux > 0:
                ...
                all_decoded = all_decoded.view(aux_size, -1, args.nout)
                
                # remove padding rows
                all_decoded = all_decoded.transpose(0,1)[loc].transpose(0,1)

                aux_losses = args.aux * sum([criterion(all_decoded[i], track_weight[targets]) for i in range(aux_size)])
...
def evaluate(data_source, label):
    ...
    with torch.no_grad(): # impacts the autograd engine and deactivate it. It will reduce memory usage and speed up computations but you won’t be able to backprop (which you don’t want in an eval script).
        ...
        for i in range(0, data_source.size(0) - 1, args.seq_len):
            ...
            (_, output, decoded), hidden, _ = net(data, hidden) #######  output; decoded => vector
            decoded = decoded.transpose(0, 1)####
            targets = targets[eff_history:].contiguous().view(-1)
            final_decoded = decoded[eff_history:].contiguous().view(-1, args.nout)
