...
log_interval =100
saved_model = 'music_trellis_2_.pt'
x = 3
###############################################################################
# Load data
###############################################################################
...
def rank(data_source, label):
    ...
    with torch.no_grad():
        ...
        for i in range(0, data_source.size(0) - 1, seq_len):
            ...
            rank_vec = model(data, hidden)[0][2][:,-1,:] # batch, ntokens [12, 1, 50704]
            ...
            for j in range(batch_size):
                ...
                if (targets[j] == 0).sum()<x:
                    track_f = tracks_future[j]
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                    score_1 = cos(rank_vec[j].unsqueeze(0), track_weight[track_f])
                    ...
                    track_f = tracks_future[j] 
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                    score_2 = cos(rank_vec[j].unsqueeze(0), track_weight[track_f])
