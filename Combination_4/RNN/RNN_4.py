...
model = RNN(29,500,len(track_dic),29,7,12,track_weight,sessions_weights)
...
try:
    for epoch in range(1, epochs + 1):
        ...
        if not best_val_loss or val_loss < best_val_loss:
                with open('music_rnn_4.pt', 'wb') as f:
                    torch.save(model, f)
                    # model.save_weights('weights/pretrained.pkl')
                    print('Saving model (new best validation) in ' + 'music_rnn_4.pt')
...
with open('music_rnn_4.pt', 'rb') as f:
    model = torch.load(f)
