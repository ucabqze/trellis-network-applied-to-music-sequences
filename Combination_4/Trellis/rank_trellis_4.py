...
log_interval =100
saved_model = 'music_trellis_4.pt'
x = 3
###############################################################################
# Load data
###############################################################################
...
class RNN(nn.Module):
    def __init__(self, emsize, nhid, ntoken, nout,nlayers, batch_size, track_weight, sessions_weights):
        ...
        self.rnn=nn.RNN(
            ...
            num_layers=nlayers,
            batch_first=True)
        self.decoder=nn.Linear(nhid,nout+10)
        ...
        for i in range(0, data_source.size(0) - 1, seq_len):
            ...
            for j in range(batch_size):
                ...
                if (label[j] >= 2).sum()<x: # no more than 5 skip
                    ...
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                    final_label = torch.cat([track_weight[track_f], torch.zeros(len(track_weight[track_f]),1)],dim = 1)
                    score_1 = cos(rank_vec[j].unsqueeze(0), final_label)              
                    ...
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                    final_label = torch.cat([track_weight[track_f], torch.zeros(len(track_weight[track_f]),1)],dim = 1)
                    score_2 = cos(rank_vec[j].unsqueeze(0), final_label)     
