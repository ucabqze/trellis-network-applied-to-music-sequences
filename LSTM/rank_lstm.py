...
data_raw, label = get_data(batch_size,sessions,seq_len)
...
def rank(data_source, label):
    ...
    with torch.no_grad():
        ...
        for i in range(0, data_source.size(0) - 1, seq_len):
            ...
            model.hidden=model.init_hidden(data_last.shape[0]) 
            rank_vec = model(data_last).squeeze() # batch, ntokens [12, 1, 50704]
            
            #music_trellis
#            hidden = model.init_hidden(data.size(0))
#            rank_vec = model(data_last,hidden)[0][2].squeeze() 
            ...
            for j in range(batch_size):
                ...
                df_future = df_future.sort_values(by = 'score',ascending=False) # 0.798054321132313
                #df_future = df_future.sample(frac=1) #0.7787248338261271
                # NDCG
