...
data_raw,label,sessions_feature  = get_data(batch_size,sessions,seq_len)
    ...
    with torch.no_grad():
        ...
        for i in range(0, data_source.size(0) - 1, seq_len):
            ...
            for j in range(batch_size):
                ...
                final_label = torch.cat([track_weight[track_f], torch.zeros(len(track_weight[track_f]),1)],dim = 1)
                score = cos(rank_vec[j].unsqueeze(0), final_label)
                ...
                #df_future = df_future.sort_values (by = 'score',ascending=False) # vec: 0.8079812787925087
...
ndcg_avg = rank(data, label,sessions_feature)
print(ndcg_avg)
