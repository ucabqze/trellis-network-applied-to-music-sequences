...
class MixSoftmax(nn.Module):
    def __init__(self, n_components, n_classes, nlasthid, ninp, decoder, dropoutl):
        ...
        self.n_classes = n_classes ### ntokens = len(corpus.dictionary)
...
class TrellisNetModel(nn.Module):
    def __init__(self, track_weight, sessions_weights, ntoken, ninp, nhid, nout, nlevels, kernel_size=2, dilation=[1],
                 dropout=0.0, dropouti=0.0, dropouth=0.0, dropoutl=0.0, emb_dropout=0.0, wdrop=0.0,
                 temporalwdrop=True, tie_weights=False, repack=False, wnorm=True, aux=True, aux_frequency=20, n_experts=0,
                 load=""):
        """
    ...
    def forward(self, input, sf, hidden, decode=False):
        """
        ...
        emb = torch.cat([emb,session_feature],dim = 2)
