...
# this is the code of Shaojie Bai, Avaliable at https://github.com/locuslab/trellisnet
...
class WeightShareConv1d(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_out, kernel_size, dropouth=0.0):
        ...
        conv1 = nn.Conv1d(input_dim, n_out, kernel_size) ### Conv1d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros')
...
class TrellisNet(nn.Module):
    def __init__(self, ninp, nhid, nout, nlevels=40, kernel_size=2, dropouth=0.0,
                 wnorm=True, aux_frequency = 3, dilation=[1]):
        """
    ...
    def transform_input(self, X):
        # X has dimension (N, ninp, L) ### (N batches, ninp embedding size, L words in a sentence)
        batch_size = X.size(0)
        seq_len = X.size(2)
        h_size = self.h_size
    ...
    def step(self, Z, dilation=1, hc=None): 
        ...
        # Apply convolution
        out = self.full_conv(Z, dilation=dilation, hid=hid) ### forward in WeightShareConv1d, include (input, dilation, hid)
        ...
        ct = ft * ct_1 + it * gt # [12,1029,20]*[12,1029,20] # * is different from @
        ht = ot * torch.tanh(ct)

        # Put everything back to form Z (i.e., injecting input to hidden unit)
        Z = torch.cat([Z[:, :ninp], ht], dim=1) ### x + new h_size
        self.ct = ct
        return Z
    ...
    def forward(self, X, hc, aux=True):
        ...
        #full_conv.drop.mask = full_conv.drop.reset_mask(Z[:, ninp:])
        ...
        out = Z[:, -nout:].transpose(1, 2)              # Dimension (N, L, nout) (batch size, sequence length, n_out)
        hc = (Z[:, ninp:, -1:], self.ct[:, :, -1:])     # Dimension (N, h_size, L)
        ### (hid, cell)
        ### hid to the next layer (hid.size(): 100, 1400, 1, all sentence, all words, next timestep)
        ### cell to the next timestep
