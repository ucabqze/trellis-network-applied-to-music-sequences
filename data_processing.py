...
#        return input_, label
        ...
        sessions_list = preprocessing.sequence.pad_sequences(sessions_list,maxlen, padding='post',truncating='post')
        sessions_list = torch.Tensor(sessions_list).long()
        label_list = preprocessing.sequence.pad_sequences(label_list,maxlen, padding='post',truncating='post')
        ...
        # convert input shape
        input_raw = sessions_list.reshape(-1,1).squeeze()
        input_ = batchify(input_raw, batch_size, cuda = False).long()
#      
        label_raw = label_list.reshape(-1,1).squeeze()
        label = batchify(label_raw, batch_size, cuda = False).long()
